...
const (
	KITCHEN_URL = "http://127.0.0.1:8881/order"
	LISTENPORT  = ":8882"
	NR_WAITERS  = 4
	NR_TABLES   = 10
...
func (w *Waiter) returnOrderToTable(returnOrder pkg.OrderResponse) {
	...
	orderLog.Println("Order:", returnOrder.OrderID, "Match order:", correctDelivery, "Waiter:", w.waiterID, "Rating:", rating)
}

func sendOrderKitchen(order pkg.Order) {
	communicationLog.Println("Waiter nr", order.WaiterID, "sent order", order.OrderID, "to kitchen")
...
func receiveRequest(w http.ResponseWriter, r *http.Request) {
	...
	communicationLog.Println("Order received, id:", newFinishedDish.OrderID)
